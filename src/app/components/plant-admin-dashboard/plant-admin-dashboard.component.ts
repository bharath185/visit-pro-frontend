...
@Component({
  selector: 'app-plant-admin-dashboard',
  templateUrl: './plant-admin-dashboard.component.html'
})
export class PlantAdminDashboardComponent implements OnInit, OnDestroy {
  user: any = null;
  ...
  constructor(private http: HttpClient, public auth: AuthService) {
    ...
    this.plantName = this.user?.PlantName || '';
  }
  ...
  loadDashboard(): void {
    ...
    this.loading = true;
    this.http.get<any>(API + '/visitor/dashboard/plant/' + this.plantId).subscribe({
      next: m => { this.headerMetrics = m; this.buildCharts(); },
      error: () => {}
    });
    this.http.post<any[]>(API + '/visitor/list/plant/' + this.plantId, {}).subscribe({
